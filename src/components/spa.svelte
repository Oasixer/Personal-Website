<script lang="ts">
  ...
  import PortfolioPage2 from '@portfolio/Portfolio2.svelte';
  import BackToTop from '@components/components/BackToTop.svelte';
  import { choosePortfolioProj, goToPortfolio } from '@viewport';
  ...
  export let load_portfolio: string = 'default';
  export let load_resume: boolean;
  let viewport: any;
  ...
  $: curSection = getCurrentSection(y);
    ...
    if (load_resume === true){
      console.log('moving to resume');
      setTimeout(()=>{
        move_to_section_n(2);
      }, 600);
    }
    else if (load_portfolio !== "default"){
      console.log('moving to portfolio');
      setTimeout(()=>{
        move_to_section_n(1);
        $choosePortfolioProj(load_portfolio);
      }, 600);
    }
    mounted = true;
	});

  // Define a type for the section structure
  type Section = {
    component: any;
    name: string;
    excludeFromMenubar: boolean;
    height?: number;
  };
  ...
  let sections: Section[] = [
    ...
    { component: BackToTop, name: 'BackToTop', excludeFromMenubar: true }
  ];
  
  function move_to_section_n(n: number){
    // load_portfolio is an optional string corresponding to a portfolio content id (== directory name)
    console.log("moving to section: ", n);
    let y_temp=0;
    for(let i=0; i<n; i++){
